...
import moviepy.audio.fx.audio_normalize
import mutagen.id3
import pytube
...
def download_audio(youtube_url: str, directory: str, status_printer = print):
    ...
    if not os.path.exists(directory):
        print_error_with_func("Error: Download location doesn't exist", status_printer)
        return None
...
def convert_mp4_to_mp3(mp4_location: str, mp3_location: str):
    ...
    with moviepy.audio.io.AudioFileClip.AudioFileClip(filename=mp4_location) as mp4:
        normalized_mp4 = mp4.fx(audio_normalize.audio_normalize)
        ...
        normalized_mp4.write_audiofile(mp3_location, logger=None)
...
def download_video(youtube_url: str, directory: str, status_printer = print):
    if not os.path.exists(directory):
        print_error_with_func("Error: Directory doesn't exist", status_printer)
        return None

    downloaded_mp4_location = None
    try:
        # Fetch the YouTube video.
        status_printer(f"Status: Finding YouTube video with url {youtube_url}")
        youtube_video = pytube.YouTube(youtube_url)
        ...
        status_printer(f"Status: Downloading {filename}.mp4")
        downloaded_mp4_location = youtube_video_stream.download(filename=f"{filename}.mp4", output_path=directory)
    except pytube.exceptions.VideoUnavailable as e:
        print_error_with_func("Error: Video was not found or is unavailable", status_printer)
        log_error(e)

    return downloaded_mp4_location
...
def find_unique_file_location(file_name: str, file_directory: str, target_file_extension: str = "mp3"):
    ...
    file_name_count = 0
    while os.path.exists(f"{file_directory}\\{unique_file_name}.{target_file_extension}"):
        file_name_count += 1
        unique_file_name = f"{file_name} ({file_name_count})"
