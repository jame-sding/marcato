...
def download_audio(youtube_url: str, directory: str, print_status):
    ...
    try:
        ...
        print_status("Status: Finding stream")
        ...
        print_status(f"Status: Converting to mp3, using mp4 at {downloaded_mp4_location}")
        final_file_location = find_unique_file_location(filename, directory)
        print_status(f"(Debug) Status: Found final file location at {final_file_location}")
        convert_mp4_to_mp3(downloaded_mp4_location, final_file_location)
        print_status("(Debug) Status: Converted mp4 to mp3")
        ...
        # print_status("Error: Could not convert to mp3", is_error=True)
        pass
    except ValueError:
        # print_status("Error: Invalid download location", is_error=True)
        pass
    except:
        print_status("Error: Unknown (bad YouTube URL?)", is_error=True)
...
def convert_mp4_to_mp3(mp4_location: str, mp3_location: str):
    ...
    with moviepy.audio.io.AudioFileClip.AudioFileClip(filename=mp4_location) as mp4:
        mp4.write_audiofile(mp3_location, verbose=False, logger=None)
